...
const ExperienceRequirementSchema = new mongoose.Schema({
    ...
    // category_text: { type: String }, // Optional: store text for display, or look up
...
const JobPostingSchema = new mongoose.Schema({
    ...
    preferredSkills: { // Array of skill IDs
        type: [{ type: String }],
        default: []
    },
    experienceRequirements: [ExperienceRequirementSchema],
    minimumDegreeLevel: { // Stores the ID, e.g., "bachelors"
        type: String,
        required: [true, 'Minimum degree level is required.']
    },
    preferredFieldOfStudy: { // Stores the ID, e.g., "computerscience" (Optional)
        type: String,
        trim: true
    },
    jobLocation: { // Stores the ID, e.g., "newyork" or "remote"
        type: String,
        required: [true, 'Job location is required.']
    },
    
    allowsRemote: { // NEW: Explicit boolean for remote option
        type: Boolean,
        default: false
    },
    ...
    salaryRange: { // Optional, free text
        type: String,
        trim: true
    },
    postedDate: {
    ...
    isActive: { // To manage if the posting is still open
        type: Boolean,
        default: true
    }
}, { timestamps: true });
