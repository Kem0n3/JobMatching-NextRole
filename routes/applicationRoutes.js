...
const JobSeekerProfile = require('../models/JobSeekerProfile'); // For populating applicant details
const { ensureAuthenticated, ensureSeeker, ensureRecruiter } = require('../middleware/authMiddleware');
const { skillsList, degreeLevelsList, fieldsOfStudyList } = require('../config/selectData'); // For displaying profile snippets
...
router.get('/job/:jobId/applicants', ensureAuthenticated, ensureRecruiter, async (req, res, next) => {
    ...
    try {
        ...
        const rawApplications = await Application.find({ job_id: jobId })
            .populate('seeker_user_id', 'username email') // Populate basic user info
            .sort({ applicationDate: -1 });

        // Manually populate seeker profiles for each application
        const applications = [];
        for (let app of rawApplications) {
            if (app.seeker_user_id && app.seeker_user_id._id) {
                ...
                applications.push(app.toObject()); // Push as is if seeker_user_id is not populated
            }
        }
        ...
        res.render('recruiter/viewApplicants', {
            ...
            job: job.toObject(),
            applications,
            skillsList, // For displaying skill text
            degreeLevelsList, // For displaying degree text
            fieldsOfStudyList, // For displaying field text
...
router.post('/status/:applicationId', ensureAuthenticated, ensureRecruiter, async (req, res) => {
    ...
    const { newStatus } = req.body; // Expected statuses: 'Viewed', 'Under Review', 'Interviewing', 'Offered', 'Hired', 'Rejected'
    ...
    try {
        ...
        if (!application) {
            ...
            return res.redirect('back'); // Or a more specific error page
        }

        // Authorization: Ensure the job posting belongs to the current recruiter
        ...
        // Validate newStatus against the enum in ApplicationSchema (Mongoose will also do this on save)
        ...
        // Determine a safe redirect, perhaps back to the applicants page if application.job_id is available
...
router.get('/my', ensureAuthenticated, ensureSeeker, async (req, res, next) => {
    ...
    try {
        const myApplications = await Application.find({ seeker_user_id: req.user.id })
            .populate({
                path: 'job_id',
                select: 'jobTitle companyName jobLocation', // Select fields from JobPosting
                populate: { // Optional: further populate recruiter from job_id if needed for display
                    path: 'recruiter_id',
                    select: 'username' // Or companyName if it's on User model for recruiter
                }
            })
            .sort({ applicationDate: -1 });

        res.render('seeker/myApplications', { // Create this new EJS file
