// routes/matches.js
 ...
 const { calculateOverallMatchScore } = require('../services/matchingService'); // Adjust path
 ...
 router.get('/jobs', ensureAuthenticated, ensureSeeker, async (req, res) => {
     ...
     try {
         ...
         for (const job of allActiveJobs) {
             ...
             // console.log(`Job: ${job.jobTitle}, Score: ${score}`); // Detailed log
             if (score > 0.5) { // Threshold score
                 matchedJobs.push({ ...job.toObject(), matchScore: score });
             }
         }

         matchedJobs.sort((a, b) => b.matchScore - a.matchScore); // Sort by score descending
         ...
         res.render('seeker/matchedJobs', {
             ...
             jobs: matchedJobs,
             // Pass lists for potential display needs in the job snippets
 ...
 router.get('/seekers/:jobId', ensureAuthenticated, ensureRecruiter, async (req, res) => {
     ...
     try {
         ...
         const allSeekerProfiles = await JobSeekerProfile.find({}).populate('user_id', 'username email'); // Populate basic user info
         ...
         for (const seekerProfile of allSeekerProfiles) {
             ...
             // console.log(`Seeker: ${seekerProfile.fullName}, Score: ${score}`); // Detailed log
             if (score > 0.3) { // Threshold
                 matchedSeekers.push({ ...seekerProfile.toObject(), user: seekerProfile.user_id, matchScore: score }); // Add user for display
             }
         }
