...
const Application = require('../models/Application'); // Required for hasApplied check
...
// --- DEFINE MORE SPECIFIC GET ROUTES FIRST ---
...
// --- DEFINE DYNAMIC PARAMETER ROUTES (LIKE /:id) LAST AMONG GET ROUTES ---

// GET route for anyone to view a single job posting
router.get('/:id', async (req, res, next) => {
    const jobId = req.params.id;

    // Crucial: Check if jobId could be one of the specific string routes defined above.
    // If so, this handler shouldn't process it; pass to next to let other handlers try.
    // This is a defensive measure; proper ordering should handle most cases.
    if (jobId === 'new' || jobId === 'my') {
        return next();
    }
    // Also validate if jobId is a valid ObjectId format early
    if (!mongoose.Types.ObjectId.isValid(jobId)) {
    ...
    try {
        ...
        // err.kind === 'ObjectId' should be caught by the isValid check now
        next(err);
    }
});
...
router.post('/:id', ensureAuthenticated, ensureRecruiter, async (req, res, next) => {
    ...
    if (!jobTitle || jobTitle.trim() === '') errors.push({ msg: 'Job title is required.' });
    if (!companyName || companyName.trim() === '') errors.push({ msg: 'Company name is required.' });
    // ... (include all other validation checks from the POST / route)
