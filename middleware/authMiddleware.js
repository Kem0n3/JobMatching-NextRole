...
module.exports = {
    ...
    ensureGuest: function(req, res, next) { // Optional: For login/register pages if user is already logged in
        if(!req.isAuthenticated()){
            return next();
        }
    ...
    ensureSeeker: function(req, res, next) {
        ...
        res.status(403).send('Access Denied: Seeker role required. <a href="/dashboard">Go to Dashboard</a>'); // Or redirect
    },
