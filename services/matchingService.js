...
function calculateJaccardIndex(setA, setB) {
    ...
    const uniqueSetA = new Set(setA.map(s => String(s).toLowerCase().trim())); // Ensure string conversion
    const uniqueSetB = new Set(setB.map(s => String(s).toLowerCase().trim())); // Ensure string conversion
...
function calculateSkillMatch(seekerSkills, jobRequiredSkills, jobPreferredSkills) {
    ...
    if (jobPreferredSkills.length > 0) {
        ...
        preferredBonus = preferredJaccardScore * 0.25; // Max bonus of 0.25 for preferred skills
                                                     // (e.g. preferred skills are 1/4th as impactful as required)
    }
...
function calculateExperienceMatch(seekerCategoryExp, jobExpRequirements) {
    ...
    for (const req of jobExpRequirements) {
        if (!req.category_id || req.minYears === undefined) continue; // Skip malformed requirements
        ...
        if (seekerExpForCategory) {
            const seekerYears = parseFloat(seekerExpForCategory.years); // Ensure number
            const requiredYears = parseFloat(req.minYears);       // Ensure number

            if (isNaN(seekerYears) || isNaN(requiredYears)) continue; // Skip if years are not valid numbers
            ...
            // Else, 0 credit for this requirement if significantly below
        }
        // If seeker has no experience in this required category, they get 0 for this requirement )
    }

    if (relevantRequirementsCount === 0) return 1; // Still return 1 if, after filtering, no valid requirements remain
...
function calculateEducationMatch(seekerDegree, seekerField, jobMinDegree, jobPreferredField) {
    ...
    if (seekerOrdinal >= jobOrdinal && jobOrdinal !== -1) { // Ensure job has a valid degree requirement
        ...
        degreeScore = 0.7; // Or 1.0, depending on how you want to score "no requirement"
    }
